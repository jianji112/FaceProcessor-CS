...
public class VideoProcessor
{
	...
	public string LastEncodingBackend { get; private set; } = "未开始";
	...
	public static string GetEncodingCapabilitySummary()
	{
		return FfmpegRawVideoWriter.SupportsNvenc() ? "NVIDIA NVENC" : "OpenCV / CPU 回退";
	}
	...
	{
		await AudioMerger.InitializeAsync();
	...
	private bool ProcessFrames(string inputPath, string tempOutputPath, ProcessMode mode, ProcessOptions options, IProgress<double>? progress, CancellationToken cancellationToken)
	{
		...
		using IVideoFrameWriter? writer = CreateWriter(tempOutputPath, normalizedFps, outputSize);
		if (writer == null)
		{
		...
		LastEncodingBackend = writer.BackendDescription;
		Trace.WriteLine("[VideoProcessor] Writer backend: " + writer.BackendDescription);
		int frameIndex = 0;
		int faceCount = 0;
	...
	private static IVideoFrameWriter? CreateWriter(string outputPath, double fps, Size frameSize)
	{
		if (FfmpegRawVideoWriter.TryCreate(outputPath, fps, frameSize, out IVideoFrameWriter? writer, out string? ffmpegError))
		{
			return writer;
		}

		Trace.WriteLine("[VideoProcessor] NVENC unavailable, fallback to OpenCV writer: " + ffmpegError);
		if (OpenCvVideoFrameWriter.TryCreate(outputPath, fps, frameSize, out writer, out string? writerError))
		{
			return writer;
		}

		Trace.WriteLine("[VideoProcessor] OpenCV writer fallback failed: " + writerError);
		return null;
	}
